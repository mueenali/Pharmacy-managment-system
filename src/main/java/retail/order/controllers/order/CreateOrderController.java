...
public class CreateOrderController extends BaseController implements Initializable, MedicineInterface, OrderInterface {
            ...
            searchField.textProperty().addListener((observable, oldValue, newValue) -> {
                searchedData.setPredicate(medicine -> {
                    ...
                    if (medicine.getName().toLowerCase().contains(lowerCaseFilter)) {
                        return true;
                    } else return String.valueOf(medicine.getId()).toLowerCase().contains(lowerCaseFilter);
                });
            });
    ...
    public void onConfirmOrder(ActionEvent actionEvent) {
        if (validateCreateOrderInput()) {
            if (orderModel.saveOrder(currentOrder)) {
                ORDERLIST.add(currentOrder);
                ORDERLIST.clear();
                ObservableList<Order> orders;
                User user = SessionManager.getInstance().getLoggedUser();
                if (SessionManager.getInstance().getLoggedUser().isAdmin()) {
                    orders = orderModel.getOrders();
                } else {
                    orders = orderModel.getMyOrders(user);
                }
